...
//   0x14 : LSR
//
// The testbench uses a moderate baud divisor (16) so TX/RX sampling has timing margin while simulation remains fast.
...
module tb_axi_uart_top;
  ...
  localparam [AXI_DATA_WIDTH-1:0] TEST_BAUD_DIV = 32'd16;
...
`endif
  ...
  // For baud divisor 16, the RTL holds each UART bit for approximately 17 clocks.
  // 10 bits for 8N1 => approximately 170 fixed-clock cycles.
  // ---------------------------------------------------------------------------
  ...
  initial begin
    ...
    // Use a moderate divisor to provide adequate RX sampling margin.
